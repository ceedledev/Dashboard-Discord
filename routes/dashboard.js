...
router.get("/profile", checkAuth, async (req, res) => {
    ...
    let userObj = req.client.users.cache.get(req.user.id);
        ...
        const flags = {
            ...
            PARTNERED_SERVER_OWNER: 'Serveur Owner ♾',
            ...
            EARLY_VERIFIED_BOT_DEVELOPER: 'Discord Developpeur',
            EARLY_VERIFIED_DEVELOPER: 'Discord Developpeur',
            VERIFIED_DEVELOPER: 'Discord Developpeur'
        };
...
router.get("/server/:guildID", checkAuth, async (req, res) => {
    ...
    if (!server && req.user.guilds.filter(u => ((u.permissions & 2146958591) === 2146958591)).map(u => u.id).includes(req.params.guildID)) {
        return res.redirect(`https://discord.com/oauth2/authorize?client_id=${req.client.id}&scope=bot&permissions=0&guild_id=${req.params.guildID}`);  
    } else if (!server) {
